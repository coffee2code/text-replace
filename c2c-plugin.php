...
if ( ! class_exists( 'c2c_TextReplace_Plugin_049' ) ) :

abstract class c2c_TextReplace_Plugin_049 {
	...
	protected function __construct( $version, $id_base, $author_prefix, $file, $plugin_options = array() ) {
		...
		if ( ! file_exists( $file ) ) {
			die( sprintf( __( 'Invalid file specified for C2C_Plugin: %s', 'text-replace' ), $file ) );
		}
	 ...
	 * @since 036
	 */
	public function __clone() { _doing_it_wrong( __FUNCTION__, __( 'Something went wrong.', 'text-replace' ), '036' ); }
	 ...
	 * @since 036
	 */
	public function __wakeup() { _doing_it_wrong( __FUNCTION__, __( 'Something went wrong.', 'text-replace' ), '036' ); }
	...
	public function options_page_description( $localized_heading_text = '' ) {
		...
		if ( ! $this->disable_contextual_help ) {
			echo '<p class="see-help">' . __( 'See the "Help" link to the top-right of the page for more help.', 'text-replace' ) . "</p>\n";
		}
	}
	...
	public function sanitize_inputs( $inputs ) {
		...
		if ( isset( $_POST['Reset'] ) ) {
			...
			add_settings_error( 'general', 'settings_reset', __( 'Settings reset.', 'text-replace' ), 'updated' );
			unset( $_POST['Reset'] );
		} else {
			...
			foreach ( $option_names as $opt ) {
				if ( !isset( $inputs[ $opt ] ) ) {
					if ( $this->config[ $opt ]['input'] == 'checkbox' ) {
						...
						$msg = sprintf( __( 'A value is required for: "%s"', 'text-replace' ), $this->config[ $opt ]['label'] );
						...
						switch ( $this->config[ $opt ]['datatype'] ) {
							...
							case 'int':
								if ( ! empty( $val ) && ( ! is_numeric( $val ) || ( intval( $val ) != round( $val ) ) ) ) {
									$msg = sprintf( __( 'Expected integer value for: %s', 'text-replace' ), $this->config[ $opt ]['label'] );
									$error = true;
									$val = '';
								}
								break;
	...
	public function verify_config() {
		...
		foreach ( $this->required_config as $config ) {
			if ( empty( $this->$config ) ) {
				die( sprintf( __( "The plugin configuration option '%s' must be supplied.", 'text-replace' ), $config ) );
			}
		}
	...
	protected function load_textdomain() {
		load_plugin_textdomain( 'text-replace' );
	}
	...
	public function contextual_help( $contextual_help, $screen_id, $screen = null ) {
		...
		$help = '<h3>' . __( 'More Plugin Help', 'text-replace' ) . '</h3>';
		$help .= '<p class="more-help">';
		$help .= '<a title="' . esc_attr( sprintf( __( 'More information about %1$s %2$s', 'text-replace' ), $this->name, $this->version ) ) .
			'" class="thickbox" href="' . $help_url . '">' . __( 'Click for more help on this plugin', 'text-replace' ) . '</a>' .
...
HTML;
	...
	public function help_tabs_content( $screen ) {
		$screen->add_help_tab( array(
			...
			'title'   => __( 'More Help', 'text-replace' ),
	...
	public function plugin_action_links( $action_links ) {
		$settings_link = '<a href="' . $this->settings_page . '.php?page='.$this->plugin_basename.'">' . __( 'Settings', 'text-replace' ) . '</a>';
	...
	public function donate_link( $links, $file ) {
		if ( $file == $this->plugin_basename ) {
			...
			$donation_url .= urlencode( sprintf( __( 'Donation for coffee2code plugin: %s', 'text-replace' ), $this->name ) );
			$title         = __( 'Coffee fuels my coding.', 'text-replace' );
			$links[] = '<a href="' . esc_url( $donation_url ) . '" title="' . esc_attr( $title ) . '">' . __( 'Donate', 'text-replace' ) . '</a>';
		}
		return $links;
	}
...
//				echo '<div class="hr">&nbsp;</div>' . "\n";
	...
	public function options_page() {
		...
		echo '<input type="submit" name="Submit" class="button-primary" value="' . esc_attr__( 'Save Changes', 'text-replace' ) . '" />' . "\n";
		echo '<input type="submit" name="Reset" class="button" value="' . esc_attr__( 'Reset Settings', 'text-replace' ) . '" />' . "\n";
		echo '</form>' . "\n";
